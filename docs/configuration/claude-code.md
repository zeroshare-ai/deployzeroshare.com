...
This guide explains how to configure the Claude Code CLI (Anthropic's coding assistant) to route requests through ZeroShare Gateway.
...
### For Complex Auth (NTLM, Kerberos)

Claude Code doesn't natively support NTLM or Kerberos. Use ZeroShare Gateway as an LLM Gateway:

1. Configure ZeroShare Gateway to handle authentication upstream
2. Connect Claude Code to ZeroShare without authentication
3. ZeroShare handles the complex auth with Anthropic

```bash
# Claude Code connects to local gateway (no auth)
export HTTPS_PROXY="http://localhost:8080"

# Gateway handles auth with upstream
# (configured in ZeroShare's config.yaml)
```
...
For running Claude Code as a service:
...
Type=simple
User=developer
Environment="HTTPS_PROXY=https://gateway.company.com:8443"
Environment="ANTHROPIC_API_KEY=sk-ant-xxx"
Environment="SSL_CERT_FILE=/etc/ssl/certs/company-ca.crt"
